<?php
    ...
    function listIdeas(){
		...
		$stmt = $mysqli->prepare("SELECT id, idea, ideacolor FROM vpuseridea WHERE userId = ? ORDER BY DESC");
		echo $mysqli->error;
		...
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"],$GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea FROM vpuseridea WHERE id = (SELECT MAX(id) FROM vpuseridea)");
		echo $mysqli->error;
